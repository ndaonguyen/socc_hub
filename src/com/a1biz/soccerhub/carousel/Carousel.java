package com.a1biz.soccerhub.carousel;
...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
	...
	private static final float MAX_THETA = 15.0f; // 15
    private static final int SCROLL_TO_FLING_UNCERTAINTY_TIMEOUT = 250; //250
    int myVelocity = 30; // custom 
	
	// Private members		
    
    private AdapterContextMenuInfo mContextMenuInfo;
    private int mAnimationDuration = 1200;    //1200
	 ...
	 * The axe angle
	 */
    private float mTheta = (float)( 15.0f*(Math.PI/180.0));	 //(float)(15.0f*(Math.PI/180.0));	
	...
	protected boolean getChildStaticTransformation(View child, Transformation transformation) {
		...
		// Center of the view
		float centerX = (float)getWidth()/2, centerY = (float)getHeight()/20;     // **    /2
	...
	void layout(int delta, boolean animate){
        ...
        float angleUnit = 360.0f / count;   // 360  				// **
			...
			for(int i = 0; i< array.length(); i++)
			{
				...
				if(reflected){
					...
					Bitmap reflectionImage = Bitmap.createBitmap(originalImage, 0,
							height / 2, width, height / 2, matrix, false);   //   /2

					// Create a new bitmap with same width but taller to fit
					// reflection
					Bitmap bitmapWithReflection = Bitmap.createBitmap(width,
							(height + height / 2), Config.ARGB_8888);       // /2
					...
					canvas.drawRect(0, height, width, height + reflectionGap,   //  ko co chia 2
							deafaultPaint);
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        mFlingRunnable.startUsingVelocity((int) myVelocity); // velocityX
    ...
    private void Calculate3DPosition(CarouselItem child, int diameter, float angleOffset){
    	...
    	float x = - (float)(diameter/2  * android.util.FloatMath.sin(angleOffset)) + diameter/2 - child.getWidth()/2;    //2    // **
    	float z = diameter/3 * (1.0f - (float)android.util.FloatMath.cos(angleOffset));     //2
    	float y = - getHeight()/3 + (float) (z * android.util.FloatMath.sin(mTheta));       //2  // ***
